...
バージョンまでの差分を適用し、最後にマーカーを前進させます（`init` が
決して行わない唯一の変更）。追加ファイルは **skip-if-exists** なので、
編集済みファイルは保持され skip（`=`）として報告されます。

バージョンによっては、以前に配布したファイルの*内容*が変わることも
あります（例: `Dockerfile` に本番ターゲットが加わる）。これは別枠で
報告され、「そのファイルが未編集であること」がゲートになります:
`upgrade` が置き換えるのは、いずれかのフレームワークバージョンが書いた
バイト列と完全に一致している場合だけ（`~`）。少しでも編集していれば
一切手を触れずそのまま残し、テンプレートが更新された旨とともに keep
（`!`）として一覧します。新しいテンプレートが欲しい場合は、自分の変更を
退避してファイルを削除し `relayer init` を実行してください。つまり
このコマンドは、あなたの変更を上書きすることなく古いテンプレートだけを
更新できます。

対象は構造差分とマーカーのみで、composer の scripts や
...
| `(group)/`            | ルートグループ。URL セグメントを増やさずにファイルを整理。独自の `layout.psx` を持てる。 |
| `_private/`           | そのフォルダと配下すべてをルーティング対象から外す。               |
...
**ルートのコンパイル（本番）。** 既定ではルーターはリクエストごとに
`src/Pages/` を走査します。デプロイ時に `vendor/bin/relayer routes:compile`
を実行すると、可読でポータブルなスナップショットを
`var/cache/routes/routes.php` に書き出し、本番はツリーを歩く代わりに
その OPcache 済みファイル 1 つを読みます。ファイルの有無だけがゲートで、
無ければライブ走査に戻る — dev は常に最新のツリーを反映し stale に
なりません。走査時の曖昧さ（page と `route.php` の衝突、ルートグループ
...
**読み取り専用ファイルシステム。** 雛形の `Dockerfile` は `prod`
ターゲットで上の 3 ステップをすべて実行するので、イメージは実行時に
何も生成せず、書き込み不可のファイルシステムで動かせます。
`compose.yaml` にはその構成をコメントアウト済みの `app-prod`
サービスとして同梱しています: `read_only: true` に加え、まだ書き込みが
発生するパスにだけ小さな `tmpfs` を割り当てます — ETag ストア
(`var/cache/etags`)、PHP のセッションとアップロード一時ファイル
（どちらも prod イメージが `var/cache/` 配下へ移設）、そして Caddy
自身の `/data` と `/config`（FrankenPHP イメージは `XDG_DATA_HOME` /
`XDG_CONFIG_HOME` をそこに向けたうえで `VOLUME` を宣言しないため、
read-only だとこれが無いと起動すらできません）。`var/cache/`
全体にマウントしては**いけません**: コンパイル済みのルート・コンテナ・
`.psx` はイメージ内のそこにあり、空の tmpfs を被せると隠れてしまい、
アーティファクトが消すはずだったリクエストごとの走査と再ビルドに
無言で戻ります。
